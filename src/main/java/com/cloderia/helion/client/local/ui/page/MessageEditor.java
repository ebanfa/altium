 ...
 * 
 */
@Templated(value = "create-message-page.html#entityEditor")
public class MessageEditor extends EntityEditor<Message> {
	...
	public void save() {
		...
		}).create(new MessageOperation(getValue(), bus.getSessionId()));
	}
	/**
	 * 
	 */
	protected native void registerHandlers()/*-{
		...
		$wnd.$('.selectpicker').on('changed.bs.select', function (e, clickedIndex, newValue, oldValue) {
			var id = e.target.id;
        	theInstance.@com.cloderia.helion.client.local.ui.page.MessageEditor::onChangedBSSelect(Ljava/lang/String;Ljava/lang/String;)(id, clickedIndex);
        });
	}-*/;
